...
import com.github.mybatis.util.PropertiesLoader;
...
//  }
  ...
  public void afterPropertiesSet() throws Exception {
	  propLoader = new PropertiesLoader(configName);
        String masterUrl = propLoader.getProperty("masterUrl");
        if (Strings.isNullOrEmpty(masterUrl)) {
          LOG.error("cannot init {}", masterUrl);
          return;
        }
        String username = propLoader.getProperty("username");
        String password = propLoader.getProperty("password");
        // 分开读和写的接口，这样互不影响，必要时可以停止读或者写
        DruidDataSource writer = buildDruidDataSource("master", masterUrl, username, password);
        String slaveUrl = propLoader.getProperty("slaveUrl");
